...
全部测试应当通过（当前 109 项）。测试**不需要任何 API Key**，因此可以在配置密钥之前先跑一遍，确认代码本身没问题。
...
交互模式会在终端底部持续显示当前目录、上下文占用/窗口、自动压缩状态、累计估算费用、会话 ID、API 后端、模型和运行模式。等待输入时由 Prompt Toolkit 绘制稳定的单行底栏；模型流式输出和工具执行时自动切换为详细的 Live 状态栏。窄终端会优先保留上下文和模型并自动隐藏次要字段；重定向输出和一次性命令不会绘制状态栏。
