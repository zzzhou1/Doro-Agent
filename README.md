...
全部测试应当通过（当前 105 项）。测试**不需要任何 API Key**，因此可以在配置密钥之前先跑一遍，确认代码本身没问题。
...
交互模式会在终端底部持续显示当前目录、上下文占用/窗口、自动压缩状态、累计估算费用、会话 ID、API 后端、模型和运行模式。等待输入时由 Prompt Toolkit 绘制；模型流式输出和工具执行时自动切换为 Live 状态栏，因此信息始终留在输出下方。窄终端会优先保留上下文和模型并自动隐藏次要字段；重定向输出和一次性命令不会绘制状态栏。
